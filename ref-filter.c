			   ...
			   size_t *valuelen)
{
	...
	if (*p == '=') {
		...
		*valuelen = p - *valuestart;
		...
		/* key begins with "candidate" but has more chars not ')' */
		return 0;
	} else {
	...
	/* p points at either the ',', ')' or NUL after this key[=val] */
	if (*p == ',')
		p++;
	else if (*p && *p != ')')
		BUG("Why is p not NULL yet?");
			  ...
			  const char **end, int *val)
{
	...
	if (!match_format_arg_value(to_parse, candidate, end, &argval, &arglen))
		return 0;
				       ...
				       struct strbuf *err)
{
	...
	if (match_format_bool_arg(*arg, "tags", arg, &optval)) {
		if (!optval)
	...
	if (match_format_arg_value(*arg, "abbrev", arg, &argval, &arglen)) {
		char *endptr;
	...
	if (match_format_arg_value(*arg, "match", arg, &argval, &arglen)) {
		if (!arglen)
			return strbuf_addf_ret(err, -1,
					       _("value expected %s="),
					       "describe:match");
	...
	if (match_format_arg_value(*arg, "exclude", arg, &argval, &arglen)) {
		if (!arglen)
			return strbuf_addf_ret(err, -1,
					       _("value expected %s="),
					       "describe:exclude");
