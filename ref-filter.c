				...
				const char *arg, struct strbuf *err)
{
	...
	for (;;) {
		...
		if (!arg) {
			break;
		} else if (match_atom_bool_arg(arg, "tags",
					       &arg, &optval)) {
			if (!optval)
			...
			found = 1;
		} else if (match_atom_arg_value(arg, "abbrev", &arg,
						&argval, &arglen)) {
			char *endptr;
			int ret = 0;
			...
			strvec_pushf(&args, "--abbrev=%.*s",
				     (int)arglen, argval);
			found = 1;
		} else if (match_atom_arg_value(arg, "match", &arg,
						&argval, &arglen)) {
			if (!arglen)
				return strbuf_addf_ret(err, -1,
						_("value expected describe:match="));
			strvec_pushf(&args, "--match=%.*s",
				     (int)arglen, argval);
			found = 1;
		} else if (match_atom_arg_value(arg, "exclude", &arg,
						&argval, &arglen)) {
			if (!arglen)
				return strbuf_addf_ret(err, -1,
						_("value expected describe:exclude="));
			strvec_pushf(&args, "--exclude=%.*s",
				     (int)arglen, argval);
			found = 1;
		}
