...
test_expect_success 'email atoms are used with bad arg' '
	cat >expect <<-EOF &&
	fatal: unrecognized %(authoremail) argument: foo
	EOF
	test_must_fail git for-each-ref \
		--format="%(authoremail:mailmap,trim,foo)" \
		refs/heads/main 2>actual &&
	...
	EOF
	test_must_fail git for-each-ref \
		--format="%(committeremail: mailmap,trim,foo)" \
		refs/heads/main 2>actual &&
	test_cmp expect actual &&
	...
	EOF
	test_must_fail git for-each-ref \
		--format="%(taggeremail:mailmap,trim, foo )" \
		refs/tags/testtag 2>actual &&
	test_cmp expect actual &&
	...
	EOF
	test_must_fail git for-each-ref \
		--format="%(committeremail:mailmap,localpart, )" \
		refs/heads/main 2>actual &&
	test_cmp expect actual &&
	...
	EOF
	test_must_fail git for-each-ref \
		--format="%(taggeremail:trim,localpart,)" \
		refs/tags/testtag 2>actual &&
	test_cmp expect actual
'

test_expect_success 'email atom options are not properly separated' '
	# Options are not seperated or are seperated but by any other
	# special character instead of a comma
	cat >expect <<-EOF &&
	fatal: expected comma separation of options
	EOF

	test_must_fail git for-each-ref \
		--format="%(authoremail:mailmaptrim)" \
		refs/heads/main 2>actual.author &&
	test_must_fail git for-each-ref \
		--format="%(committeremail:mailmap_localpart)" \
		refs/heads/main 2>actual.committer &&
	test_must_fail git for-each-ref \
		--format="%(taggeremail:mailmap;localpart trim)" \
		refs/heads/testag 2>actual.tagger &&

	test_cmp expect actual.author &&
	test_cmp expect actual.committer &&
	test_cmp expect actual.tagger
