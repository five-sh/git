...
test_expect_success 'reflog ident email,trim atom' '
	cat >expect <<-\EOF &&
	committer@example.com
	committer@example.com
	committer@example.com
	...
	committer@example.com
	EOF
	git for-each-ref --format="%(reflog:email,trim)" \
		refs/heads/t-reflog >actual &&
	test_cmp expect actual
'

test_expect_success 'reflog ident email,localpart atom' '
	cat >expect <<-\EOF &&
	committer
	committer
	committer
	...
	committer
	EOF
	git for-each-ref --format="%(reflog:email,localpart)" \
		refs/heads/t-reflog >actual &&
	test_cmp expect actual
'
